...
3. In the Nummion repository's Actions secrets, add `HOMEBREW_TAP_TOKEN`: a fine-grained GitHub token restricted to `JangoCG/homebrew-tap`, with **Contents: read and write**. The tap already exists. Both Homebrew (`Casks/nummion.rb`) and Scoop (`bucket/nummion.json`) use it. GitHub's automatic `GITHUB_TOKEN` cannot write to a different repository. Never commit or place this token in command-line arguments.
4. Keep GitHub Actions enabled. The release job declares `contents: write` for release uploads and `id-token: write` for keyless signing; CI has read-only permissions.
...
CI runs Go checks and the race detector on Linux, macOS, and Windows, exercises the installers offline, and builds the full snapshot. Windows installer integration tests use the freshly built Windows executable. Local macOS validation cannot replace a Windows runner or actual hosted signing/publication.

## Publish

After the final commit is pushed and CI is green, create and push an annotated semantic-version tag:
...
Only execute these commands when intentionally publishing. The tag triggers the release workflow. It reruns CI before GoReleaser builds the artifacts, signs `checksums.txt` with keyless Sigstore, publishes the GitHub Release, and updates the tap. Stable releases require the tap token. A tag such as `v0.1.0-rc.1` creates a prerelease and skips tap updates; it does not replace GitHub's latest stable release.
...
Sigstore proves the release's workflow identity; it does not replace operating-system code signing. Optional macOS signing/notarization is configured through these Actions secrets:
